...
***** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  Conventional top level example.
        ...
        cmnd('djangoBasesPrep', comment=" # Prep Django before start")
...
#+end_example
        ...
        reposEnsure().pyCmnd()
        djangoBasesPrep().pyCmnd()
        reactBasesPrep().pyCmnd()

        return cmndOutcome


####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "reposEnsure" :comment "" :extent "verify" :ro "cli" :parsMand "" :parsOpt "" :argsMin 0 :argsMax 0 :pyInv ""
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<reposEnsure>>  =verify= ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class reposEnsure(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
    cmndArgsLen = {'Min': 0, 'Max': 0,}
...
#+RESULTS:
        ...
        bxReposBase = pathlib.Path("/bisos/git/bxRepos")
        cliGuiBase = bxReposBase / pathlib.Path("cliGui")

        if not cliGuiBase.exists():
            cliGuiBase.mkdir()

        webCliGuiBase = cliGuiBase / pathlib.Path("webCliGui")

        if not webCliGuiBase.exists():
            if b.subProc.WOpW(invedBy=self, cd=str(cliGuiBase), log=1).bash(
f"""git clone https://github.com/cliGui/webCliGui.git"""
            ).isProblematic():  return(b_io.eh.badOutcome(cmndOutcome))
        else:
            if b.subProc.WOpW(invedBy=self, cd=str(webCliGuiBase), log=1).bash(
f"""git pull"""
            ).isProblematic():
                b_io.eh.badOutcome(cmndOutcome,)
                # return(b_io.eh.badOutcome(cmndOutcome))

        csLibBase = cliGuiBase / pathlib.Path("csLib")

        if not csLibBase.exists():
            if b.subProc.WOpW(invedBy=self, cd=str(csLibBase), log=1).bash(
f"""git clone https://github.com/cliGui/csLib.git"""
            ).isProblematic():  return(b_io.eh.badOutcome(cmndOutcome))
        else:
            if b.subProc.WOpW(invedBy=self, cd=str(csLibBase), log=1).bash(
f"""git pull"""
...
####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "djangoBasesPrep" :comment "" :extent "verify" :ro "cli" :parsMand "" :parsOpt "" :argsMin 0 :argsMax 0 :pyInv ""
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<djangoBasesPrep>>  =verify= ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class djangoBasesPrep(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
    cmndArgsLen = {'Min': 0, 'Max': 0,}
...
#+RESULTS:
        ...
        if b.subProc.WOpW(invedBy=self, cd=str(webCliGuiBase / pathlib.Path("webcligui_api")), log=1).bash(
f"""pip install -e ."""
        ).isProblematic():  return(b_io.eh.badOutcome(cmndOutcome))
...
f"""pip install -e ."""
        ...
        if b.subProc.WOpW(invedBy=self, cd=str(webCliGuiBase / pathlib.Path("server")), log=1).bash(
f"""./manage.py migrate"""
        ).isProblematic():  return(b_io.eh.badOutcome(cmndOutcome))
...
####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "reactBasesPrep" :comment "" :extent "verify" :ro "cli" :parsMand "" :parsOpt "" :argsMin 0 :argsMax 0 :pyInv ""
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<reactBasesPrep>>  =verify= ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class reactBasesPrep(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
    cmndArgsLen = {'Min': 0, 'Max': 0,}
